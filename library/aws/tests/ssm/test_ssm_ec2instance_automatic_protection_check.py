...
class TestSsmEc2InstanceAutomaticProtection:
    ...
    def test_when_ssm_managed_instance_has_termination_protection_enabled(self):
    ...
    def test_when_ssm_managed_instance_has_termination_protection_disabled(self):
        self.mock_ec2.get_paginator.return_value.paginate.return_value = [
    ...
    def test_when_running_instances_are_not_managed_by_ssm(self):
        self.mock_ec2.get_paginator.return_value.paginate.return_value = [
    ...
    def test_when_no_running_instances_exist(self):
        self.mock_ec2.get_paginator.return_value.paginate.return_value = [
    ...
    def test_when_ec2_client_raises_client_error(self):
        ...
        assert "Unexpected error during check execution" in report.resource_ids_status[0].summary

    def test_when_termination_protection_attribute_is_missing(self):
        self.mock_ec2.get_paginator.return_value.paginate.return_value = [
            {"Reservations": [{"Instances": [{"InstanceId": "i-567890", "State": {"Name": "running"}}]}]}
        ]
        self.mock_ec2.describe_instance_attribute.return_value = {}  # No DisableApiTermination key
        self.mock_ssm.get_paginator.return_value.paginate.return_value = [
            {"InstanceInformationList": [{"InstanceId": "i-567890"}]}
        ]
        report = self.check.execute(self.mock_session)
        assert report.status == CheckStatus.FAILED  # Changed from UNKNOWN
        assert report.resource_ids_status[0].status == CheckStatus.FAILED
        assert "does not have termination protection enabled" in report.resource_ids_status[0].summary
